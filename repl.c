...
static char *ast_to_string(ASTNode *node) {
  ...
  case AST_LITERAL: {
    ...
    if (v->type == INT) {
      char buf[64];
      snprintf(buf, sizeof(buf), "%lld", (long long)v->as.int_value);
      return k_strdup(buf);
    }
    if (v->type == FLOAT) {
      char buf[64];
      snprintf(buf, sizeof(buf), "%g", v->as.float_value);
      return k_strdup(buf);
    }
    if (v->type == VECTOR) {
      // string literal
      ...
      for (size_t i = 0; i < v->as.vector->length; i++) {
        if (v->as.vector->items[i].type != CHAR) {
          is_str = 0;
          break;
        }
      }
      if (is_str) {
        ...
        for (size_t i = 0; i < n; i++)
          s[1 + i] = v->as.vector->items[i].as.char_value;
        s[1 + n] = '"';
        s[2 + n] = '\0';
        return s;
      }
  ...
  case LAMBDA: {
    ...
    if (lam->param_count > 0) {
      ...
      for (int i = 0; i < lam->param_count; i++)
        len += strlen(lam->params[i]);
      len += (size_t)(lam->param_count > 1 ? lam->param_count - 1
                                           : 0); // ; between params
    }
    char **bodies = NULL;
    if (lam->body_count > 0) {
...
void print(KObj *obj) {
  ...
  if (obj->type == VECTOR && obj->as.vector->length == 1 &&
      !is_char_vector(obj)) {
    putchar(',');
  ...
  if (simple) {
    if (all_strings) {
      ...
      for (size_t i = 0; i < obj->as.vector->length; i++) {
        char *s = kobj_to_string(&obj->as.vector->items[i]);
        printf("%s", s);
        if (!all_syms && i + 1 < obj->as.vector->length)
          putchar(' ');
        free(s);
      }
      putchar('\n');
...
static void time_and_print_average(const char *expr, long runs,
                                   int repl_prompt) {
  if (runs <= 0)
    runs = 1;
