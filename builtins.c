...
static double as_double(KObj *o) {
  switch (o->type) {
  case FLOAT:
    return o->as.float_value;
  case INT:
    return (double)o->as.int_value;
  default:
    return (double)(unsigned char)o->as.char_value;
  }
}
...
static int64_t as_int(KObj *o) {
  switch (o->type) {
  case INT:
    return o->as.int_value;
  case CHAR:
    return (int64_t)(unsigned char)o->as.char_value;
  case FLOAT:
    return (int64_t)o->as.float_value;
  default:
    return 0;
  }
}
...
static KObj *apply_dict_binary(KObj *left, KObj *right,
  ...
  if (left->type == DICT && right->type == DICT) {
    KObj *vals =
        apply_binary(left->as.dict->values, right->as.dict->values, op);
    if (vals->type == NIL)
      return vals;
...
static KObj *op_add(KObj *left, KObj *right) {
  if (!are_numbers(left, right))
    return create_nil();
  if (left->type == PINF || right->type == PINF)
    return create_pinf();
  if (left->type == NINF || right->type == NINF)
    return create_ninf();
...
static KObj *op_sub(KObj *left, KObj *right) {
  if (!are_numbers(left, right))
    return create_nil();
  if (left->type == PINF || right->type == NINF)
    return create_pinf();
  if (left->type == NINF || right->type == PINF)
    return create_ninf();
  if (left->type == FLOAT || right->type == FLOAT)
    return create_float(as_double(left) - as_double(right));
  return create_int(as_int(left) - as_int(right));
...
static KObj *op_max(KObj *left, KObj *right) {
  ...
  if (left->type == FLOAT || right->type == FLOAT)
    return create_float(as_double(left) > as_double(right) ? as_double(left)
                                                           : as_double(right));
...
static KObj *op_lt(KObj *left, KObj *right) {
  ...
  if (left->type == SYM && right->type == SYM)
    return create_int(strcmp(left->as.symbol_value, right->as.symbol_value) <
                      0);
  return create_int(0);
}

static KObj *op_gt(KObj *left, KObj *right) { return op_lt(right, left); }
...
static KObj *op_sin(KObj *left, KObj *value) {
  ...
  if (!is_number(value))
    return create_nil();
  if (value->type == PINF || value->type == NINF)
    return create_nil();
...
static KObj *op_exp(KObj *left, KObj *value) {
  ...
  double r = exp(v);
  if (isinf(r))
    return r > 0 ? create_pinf() : create_ninf();
  if (isnan(r))
    return create_nil();
...
static KObj *op_pow(KObj *left, KObj *right) {
  ...
  double expv;
  if (right->type == FLOAT)
    expv = right->as.float_value;
  else
    expv = (double)as_int(right);
...
static KObj *op_not(KObj *left, KObj *value) {
  ...
  if (value->type == PINF || value->type == NINF)
    return create_int(0);
  if (value->type == FLOAT)
    return create_int(as_double(value) == 0.0);
...
KObj *k_eq(KObj *left, KObj *right) { return apply_binary(left, right, op_eq); }

KObj *k_exp(KObj *value) { return apply_binary(value, value, op_exp); }

KObj *k_rand(KObj *value) { return apply_binary(value, value, op_rand1); }

KObj *k_sin(KObj *value) { return apply_binary(value, value, op_sin); }

KObj *k_cos(KObj *value) { return apply_binary(value, value, op_cos); }

KObj *k_abs(KObj *value) { return apply_binary(value, value, op_abs); }

KObj *k_sqrt(KObj *value) { return apply_binary(value, value, op_sqrt); }

KObj *k_pow(KObj *left, KObj *right) {
  return apply_binary(left, right, op_pow);
}

KObj *k_log(KObj *value) { return apply_binary(value, value, op_log); }
...
KObj *k_floor(KObj *value) { return apply_binary(value, value, op_floor); }
...
KObj *k_not(KObj *value) { return apply_binary(value, value, op_not); }
...
static int asc_cmp(KObj *a, KObj *b, bool *domain) {
  if (is_number(a) && is_number(b)) {
    if (a->type == PINF || b->type == NINF)
      return 1;
    if (a->type == NINF || b->type == PINF)
      return -1;
  ...
  if (a->type == VECTOR && b->type == VECTOR) {
    ...
    for (size_t i = 0; i < n; i++) {
      int cmp =
          asc_cmp(&a->as.vector->items[i], &b->as.vector->items[i], domain);
      if (*domain)
        return 0;
      if (cmp != 0)
        return cmp;
...
KObj *k_desc(KObj *value) { return k_rev(k_asc(value)); }
...
static KObj *build_enum_row(int64_t *dims, size_t dims_len, size_t idx,
                            int64_t total) {
  KObj *row = create_vec((size_t)total);
  if (total == 0 || dims[idx] == 0)
    return row;
  int64_t repeat_after = 1;
  for (size_t j = idx + 1; j < dims_len; j++)
    repeat_after *= dims[j];
  if (repeat_after == 0)
    return row;
...
KObj *k_key(KObj *left, KObj *right) {
  if (left->type != VECTOR) {
    if (left->type == DICT || left->type == VERB || left->type == ADVERB ||
        left->type == LAMBDA) {
      printf("^domain\n");
      return create_nil();
    }
  ...
  if (right->type == DICT || right->type == VERB || right->type == ADVERB ||
      right->type == LAMBDA) {
    printf("^domain\n");
    return create_nil();
  }
...
static KObj *build_shape(FlatIter *it, int64_t *dims, size_t dim_idx,
                         size_t dims_len) {
...
static KObj *drop_fn(KObj *fn, KObj *src) {
  ...
  for (size_t i = 0; i < vec->as.vector->length; i++) {
    ...
    switch (r->type) {
    case INT:
      drop = (r->as.int_value != 0);
      break;
    case FLOAT:
      drop = (r->as.float_value != 0.0);
      break;
    case CHAR:
      drop = (r->as.char_value != 0);
      break;
    case NIL:
      drop = false;
      break;
...
KObj *k_concat(KObj *left, KObj *right) {
  ...
  if (is_char_vector(left) && is_char_vector(right)) {
    ...
    for (size_t i = 0; i < llen; i++) {
      out[i] = left->as.vector->items[i];
      out[i].ref_count = 1;
    }
    for (size_t i = 0; i < rlen; i++) {
      out[llen + i] = right->as.vector->items[i];
      out[llen + i].ref_count = 1;
    }
